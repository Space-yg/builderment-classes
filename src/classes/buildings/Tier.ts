...
/** The options of the tiers */
interface TiersOptions {
    [/** The tiers. They must be consecutive integers */ tier: number]: {
        /**
         * The output increase added to the previous tier.
         * Example: from tier 2 to tier 3 Extractor, the increase is 1.333
         */
        output: number
        /**
         * The price of that tier
         * - Default is 0 for gold and gems
         */
        price: PriceOptions
        /** The URL to the image of that tier. If image is not provided, it will use the image from the previous tier. The smallest tier must have an image */
        image: string
    }
}

/** The options of the optional tiers */
interface OptionalTiersOptions {
    [/** The tiers. They must be consecutive integers */ tier: number]: {
        /**
         * The output increase added to the previous tier.
         * Example: from tier 2 to tier 3 Extractor, the increase is 1.333
...
export { TiersOptions, OptionalTiersOptions }
...
export class Tier {
    ...
    constructor(options: {
        ...
        var tiersKeys: number[] = new Array(Object.keys(options.tiers).length)
        Object.keys(options.tiers).forEach((tier, i) => tiersKeys[i] = Number(tier))
        var lastImage = options.tiers[Math.min(...tiersKeys)].image
        if (lastImage === undefined) throw new Error("The smallest tier must have an image")
        for (let i = Math.min(...tiersKeys); i < Math.max(...tiersKeys); i++) {
            // Check if tier exists
            if (!options.tiers[i]) throw new Error(`The number ${i} is missing from the tiers. The tiers must include all integers from the smallest value (${Math.min(...tiersKeys)}) to the largest value (${Math.max(...tiersKeys)}).`)
            this.tiers[i] = {
            ...
            if (typeof options.tiers[i].price !== "undefined") {
                if (options.tiers[i].price!.gold) this.tiers[i].price.gold = options.tiers[i].price!.gold!
                if (options.tiers[i].price!.gems) this.tiers[i].price.gems = options.tiers[i].price!.gems!
            }
            // Image
            if (typeof options.tiers[i].image === "undefined") this.tiers[i].image = lastImage
            else lastImage = options.tiers[i].image!
        }
     ...
     * @param tier The tier
     */
    getTotalOutput(tier: number): number {
        var total = 1
        if (tier < this.minTier) throw new Error("Tier cannot be less than minimum tier.")
        if (tier > this.maxTier) throw new Error("Tier cannot be greater than maximum tier.")
